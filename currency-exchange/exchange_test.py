...
class CurrencyExchangeTest(unittest.TestCase):
    ...
    def test_exchange_money(self):
        input_data: list[tuple[int, float]] = [(100000, 0.8), (700000, 10.0)]
        output_data = [125000, 70000]

        for variant, (args, output) in enumerate(zip(input_data, output_data), start=1):
            with self.subTest(f"variation #{variant}", args=args, output=output):
                self.assertAlmostEqual(exchange_money(*args), output)
    ...
    def test_get_change(self):
        ...
        for variant, (args, output) in enumerate(zip(input_data, output_data), start=1):
            with self.subTest(f"variation #{variant}", args=args, output=output):
                self.assertAlmostEqual(get_change(*args), output)
    ...
    def test_get_value_of_bills(self):
        ...
        for variant, (args, output) in enumerate(zip(input_data, output_data), start=1):
            with self.subTest(f"variation #{variant}", args=args, output=output):
                self.assertEqual(get_value_of_bills(*args), output)
    ...
    def test_get_number_of_bills(self):
        ...
        for variant, (args, output) in enumerate(zip(input_data, output_data), start=1):
            with self.subTest(f"variation #{variant}", args=args, output=output):
                self.assertEqual(get_number_of_bills(*args), output)
    ...
    def test_get_leftover_of_bills(self):
        ...
        for variant, (args, output) in enumerate(zip(input_data, output_data), start=1):
            with self.subTest(f"variation #{variant}", args=args, output=output):
                self.assertAlmostEqual(get_leftover_of_bills(*args), output)
    ...
    def test_exchangeable_value(self):
        ...
        for variant, (args, output) in enumerate(zip(input_data, output_data), start=1):
            with self.subTest(f"variation #{variant}", args=args, output=output):
                self.assertEqual(exchangeable_value(*args), output)
