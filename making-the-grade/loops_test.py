...
class MakingTheGradeTest(unittest.TestCase):
    ...
    def test_round_scores(self):
        ...
        for variant, (student_scores, result) in enumerate(data, start=1):
            ...
            with self.subTest(
                f"variation #{variant}", input=student_scores, output=result
            ):
                self.assertEqual(
                    sorted(round_scores(student_scores)),
                    sorted(result),
                    msg=error_message,
                )
    ...
    def test_count_failed_students(self):
        ...
        for variant, (student_scores, result) in enumerate(data, start=1):
            ...
            with self.subTest(
                ...
                self.assertEqual(
                    count_failed_students(student_scores), result, msg=error_message
                )
    ...
    def test_student_ranking(self):
        ...
        for variant, (params, result) in enumerate(data, start=1):
            error_message = (
                f"Expected: {result} but the rankings were compiled incorrectly."
            )
            with self.subTest(f"variation #{variant}", input=params, output=result):
    ...
    def test_perfect_score(self):
        ...
        for variant, (student_info, result) in enumerate(data, start=1):
            error_message = (
                f"Expected: {result} but got something different for perfect scores."
            )
            with self.subTest(
                f"variation #{variant}", input=student_info, output=result
            ):
